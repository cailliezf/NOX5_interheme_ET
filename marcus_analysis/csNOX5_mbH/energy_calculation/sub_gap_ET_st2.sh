...
mkdir csNOX5_mbH_${INPUT}
cd csNOX5_mbH_${INPUT}
...
cp ../start/eq_st2/step5_input.str .
cp ../start/eq_st2/step5_input.colvar.str .
cp ../psf/*.psf .
cp ../start/eq_st2/step5_assembly.pdb .
cp ../start/eq_st2/step5_assembly.crd .
cp -r ../start/eq_st2/toppar .
...
for ((RUN=$START;RUN<=$END;RUN+=1)) ; do  # loop on MD runs
	...
	cp simulation_folder/nox5_MD_st2_${RUN}.vel .
	cp simulation_folder/nox5_MD_st2_${RUN}.coor .
	cp simulation_folder/nox5_MD_st2_${RUN}.xsc .
	...
	# Upper leaflet POPE contribution
        cat  ${INPUT}.inp | sed "s/nox5st1_fad.psf/nox5st1_lipid_pope_upper_leaflet.psf/" > tmp9
       	cat tmp9 | sed "s/nrj_st2_e1_fad_${RUN}/nrj_st2_e1_POPE_upper_${RUN}/"  > tmp10
	mv tmp10 ${INPUT}.inp
	...
	cat  ${INPUT}.inp | sed "s/nox5st1_lipid_pope_upper_leaflet.psf/nox5st1_lipid_popg_upper_leaflet.psf/" > tmp11
       	cat tmp11 | sed "s/nrj_st2_e1_POPE_upper_${RUN}/nrj_st2_e1_POPG_upper_${RUN}/"  > tmp12
	mv tmp12 ${INPUT}.inp
	...
	cat  ${INPUT}.inp | sed "s/nox5st1_lipid_popg_upper_leaflet.psf/nox5st1_lipid_pope_lower_leaflet.psf/" > tmp13
       	cat tmp13 | sed "s/nrj_st2_e1_POPG_upper_${RUN}/nrj_st2_e1_POPE_lower_${RUN}/"  > tmp14
	mv tmp14 ${INPUT}.inp
	...
	cat  ${INPUT}.inp | sed "s/nox5st1_lipid_pope_lower_leaflet.psf/nox5st1_lipid_popg_lower_leaflet.psf/" > tmp15
       	cat tmp15 | sed "s/nrj_st2_e1_POPE_lower_${RUN}/nrj_st2_e1_POPG_lower_${RUN}/"  > tmp16
	mv tmp16 ${INPUT}.inp
	...
	cat  ${INPUT}.inp | sed "s/nox5st1_lipid_popg_lower_leaflet.psf/nox5st1_protein_tm_part.psf/" > tmp17
       	cat tmp17 | sed "s/nrj_st2_e1_POPE_lower_${RUN}/nrj_st2_e1_TM_protein_${RUN}/"  > tmp18
	mv tmp18 ${INPUT}.inp
	...
	cat  ${INPUT}.inp | sed "s/nox5st1_protein_tm_part.psf/nox5st1_protein_dh_part.psf/" > tmp19
       	cat tmp19 | sed "s/nrj_st2_e1_TM_protein_${RUN}/nrj_st2_e1_DH_protein_${RUN}/"  > tmp20
	mv tmp20 ${INPUT}.inp
	...
	cat  ${INPUT}.inp | sed "s/nox5st1_protein_dh_part.psf/nox5st1_counterions_na.psf/" > tmp21
       	cat tmp21 | sed "s/nrj_st2_e1_DH_protein_${RUN}/nrj_st2_e1_NA_ions_${RUN}/"  > tmp22
	mv tmp22 ${INPUT}.inp
	...
	cat  ${INPUT}.inp | sed "s/nox5st1_counterions_na.psf/nox5st1_counterions_cl.psf/" > tmp23
       	cat tmp23 | sed "s/nrj_st2_e1_NA_ions_${RUN}/nrj_st2_e1_CL_ions_${RUN}/"  > tmp24
	mv tmp24 ${INPUT}.inp
	...
	cat  ${INPUT}.inp | sed "s/nox5st1_counterions_cl.psf/nox5st1_water.psf/" > tmp25
       	cat tmp25 | sed "s/nrj_st2_e1_CL_ions_${RUN}/nrj_st2_e1_water_${RUN}/"  > tmp26
	mv tmp26 ${INPUT}.inp
        ...
        cat tmp27 | sed "s/nrj_st2_e1_water_${RUN}/nrj_st2_e1_propionate_${RUN}/"  > tmp28
        mv tmp28 ${INPUT}.inp
        namd2 ${INPUT}.inp > ${INPUT}_PROP_e1.out${RUN}

        # Total energy of the system 
        cat  ${INPUT}.inp | sed "s/nox5st1_propionate.psf/nox5st1.psf/" > tmp29
        cat tmp29 | sed "s/nrj_st2_e1_propionate_${RUN}/nrj_st2_e1_all_${RUN}/"  > tmp30
        mv tmp30 ${INPUT}.inp
...
for ((I=1;I<=$nline;I+=1)) ; do    # Loop over the snapshots of run $RUN and store energy in etot
	...
	cat  ${INPUT}.inp | sed "s/nox5st2_lipid_pope_upper_leaflet.psf/nox5st2_lipid_popg_upper_leaflet.psf/" > tmp11
       	cat tmp11 | sed "s/nrj_st2_e2_POPE_upper_${RUN}/nrj_st2_e2_POPG_upper_${RUN}/"  > tmp12
	mv tmp12 ${INPUT}.inp
	...
	cat  ${INPUT}.inp | sed "s/nox5st2_lipid_popg_upper_leaflet.psf/nox5st2_lipid_pope_lower_leaflet.psf/" > tmp13
       	cat tmp13 | sed "s/nrj_st2_e2_POPG_upper_${RUN}/nrj_st2_e2_POPE_lower_${RUN}/"  > tmp14
	mv tmp14 ${INPUT}.inp
	...
	cat  ${INPUT}.inp | sed "s/nox5st2_lipid_pope_lower_leaflet.psf/nox5st2_lipid_popg_lower_leaflet.psf/" > tmp15
       	cat tmp15 | sed "s/nrj_st2_e2_POPE_lower_${RUN}/nrj_st2_e2_POPG_lower_${RUN}/"  > tmp16
	mv tmp16 ${INPUT}.inp
	...
	cat  ${INPUT}.inp | sed "s/nox5st2_lipid_popg_lower_leaflet.psf/nox5st2_protein_tm_part.psf/" > tmp17
       	cat tmp17 | sed "s/nrj_st2_e2_POPE_lower_${RUN}/nrj_st2_e2_TM_protein_${RUN}/"  > tmp18
	mv tmp18 ${INPUT}.inp
	...
	cat  ${INPUT}.inp | sed "s/nox5st2_protein_tm_part.psf/nox5st2_protein_dh_part.psf/" > tmp19
       	cat tmp19 | sed "s/nrj_st2_e2_TM_protein_${RUN}/nrj_st2_e2_DH_protein_${RUN}/"  > tmp20
	mv tmp20 ${INPUT}.inp
