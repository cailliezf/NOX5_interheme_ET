...
file_st1_e1="../data/nrj_st1_e1_1_40.dat"   # Files containing the energies of state 1 for MD performed on state 1
file_st1_e2="../data/nrj_st1_e2_1_40.dat"   # Files containing the energies of state 2 for MD performed on state 1
file_st2_e1="../data/nrj_st2_e1_1_40.dat"   # Files containing the energies of state 1 for MD performed on state 2
file_st2_e2="../data/nrj_st2_e2_1_40.dat"   # Files containing the energies of state 2 for MD performed on state 2
# Contribution to be computed
contr="All"
# Starting of production phase
xmin=3000                           # begining of production phase
xmax2=6000
...
for i in range(len(deltae1)):
    dd1[i]=((np.sum(deltae1[i,5:16])-10*deltae1[i,2])-deltae1[i,1]) # *100/deltae1[i,1]
    dd2[i]=((np.sum(deltae2[i,5:16])-10*deltae2[i,2])-deltae2[i,1]) # *100/deltae2[i,1]

if (showplot):
    plt.figure()
    plt.plot(range(len(deltae1)),dd1,label="State 1", c='r')
    plt.plot(range(len(deltae2)),dd2,label="State 2", c='b')
...
try:
    with open(f'deltaG_csNOX5_mbW_{xmin}-{xmax2}.pkl', 'rb') as fichier:
...
with open(f'deltaG_csNOX5_mbW_{xmin}-{xmax2}.pkl', 'wb') as fichier:
